...
function main() {
    ...
    // FAKE CAMERA //
    const fakeCamera = new THREE.PerspectiveCamera( 45, 1, 0.1, 10 );
    ...
    const intensity1 = 200;
    const light1 = new THREE.DirectionalLight(color1, intensity1);
    light1.position.set(0, 0, 10);
    light1.target.position.set(0, 0, 0);
    fakeScene.add(light1);
    fakeScene.add(light1.target);

    //TORUS
    const torus = new THREE.Mesh(   new THREE.TorusGeometry(2, 1, 16, 100), 
                                    new THREE.MeshPhongMaterial( { color: 'rgba(0, 0, 0, 1)' } ) 
                                );
